...
public class myJoystick : MonoBehaviour
{
    ...
    */
	...
	private static string joysticksTag = "myJoystick";
	/* A static collection of all joysticks */
	private static List<myJoystick> joysticks;
	/* Has the joysticks collection been enumerated yet? */
	private static bool enumeratedmyJoysticks = false;
	...
	private void Awake()
	{
		...
		if (gui.texture == null)
		{
			Debug.LogError("myJoystick object requires a valid texture!");
			gameObject.SetActive( false);
			return;
		}
		
		if (!enumeratedmyJoysticks)
		{
			try
			{
				/* Collect all joysticks in the game, so we can relay finger latching messages */
				GameObject[] objs = GameObject.FindGameObjectsWithTag(joysticksTag);
				joysticks = new List<myJoystick>(objs.Length);
				foreach (GameObject obj in objs)
				{
					myJoystick newmyJoystick = obj.GetComponent<myJoystick>();
					if (newmyJoystick == null)
					{
						throw new NullReferenceException("myJoystick gameObject found without a suitable myJoystick component.");
					}
					joysticks.Add(newmyJoystick);
				}
				enumeratedmyJoysticks = true;
			}
			catch (Exception exp)
			{
				Debug.LogError("Error collecting myJoystick objects: "+exp.Message);
				throw;
			}
		}
                    ...
                    */
					...
					if (touchPad)
					{
						...
						/*position = new Vector2
							(
								Mathf.ClampMagnitude(((touch.position.x - fingerDownPos.x) / (touchZone.width / 2), -1, 1),
								Mathf.Clamp((touch.position.y - fingerDownPos.y) / (touchZone.height / 2), -1, 1)
								);*/
					     position = Vector2.ClampMagnitude(new Vector2(touch.position.x,touch.position.y),0.5f);
					}
					else
					{
